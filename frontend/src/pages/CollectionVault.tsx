...
export function CollectionVaultPage() {
  ...
  const rareCount = items.filter(
    (e) => (RARITY_ORDER[e.card.rarity] ?? 0) >= 4,
  ).length;

  const filtered = useMemo(() => {
    let out = items;
    if (tab === "rare")
      out = items.filter((e) => (RARITY_ORDER[e.card.rarity] ?? 0) >= 4);
  ...
  return (
    <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-10">
      ...
      <div className="mb-8">
        ...
        <h1 className="font-display font-black text-4xl lg:text-5xl mb-2">
          Mi colección
        </h1>
        <p className="text-white/55">
          Cartas desbloqueadas tras pago verificado
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        <StatCard
          ...
          hint={`${items.reduce((a, e) => a + e.quantity, 0)} totales`}
        />
        <StatCard
          icon={TrendingUp}
          label="Valor de bóveda"
          value={formatPrice(totalValue)}
          hint="Suma de mercado"
        />
        <StatCard
          icon={Sparkles}
          label="Cartas raras"
          value={rareCount.toString()}
          hint="Elite, Apex, Ascendant, Eternal"
        />
      </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-24 rounded-2xl glass">
          ...
          <p className="text-white/55 mb-6 max-w-md mx-auto">
            Empieza tu colección comprando cartas en el marketplace. Cada carta
            que adquieras quedará registrada aquí.
          </p>
